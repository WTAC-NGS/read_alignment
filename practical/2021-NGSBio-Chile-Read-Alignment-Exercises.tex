...
can close the IGV software for now; we will use it later in this tutorial to
...
sequencing reads to the \textit{Mus Musculus} reference genome. We will
align genomic sequence (from Whole-Genome Sequencing) from a mouse
embryo that was mutagenised at the one-cell stage using
CRISPR-Cas9 and a gRNA targeting an exon of the Tyr gene. The successful
mutation of the gene results in the deletion one or both alleles. A bi-allelic null
...
Go to the \texttt{ref} directory, which contains the fasta files of the
reference genome:
...
the data has been aligned. A single chromosome from the mouse genome is contained in
...
\end{tcolorbox}
    ...
    Similar to a BAM file, an index file is required to allow fast retrieval of data,
You should check for the presence of fasta indexes for the genome in the `ref' directory:
...
\end{tcolorbox}
    ...
    Finally index the sorted BAM file using \texttt{samtools index} command.
...
avoid writing intermediate files. To do this type:
...
We will use a program called \texttt{MarkDuplicates} that is part of Picard
...
In your web browser open the file called \texttt{md5638\_plot/index.html} to view the
QC information and answer the following questions:

\textbf{Q9: How many reads have zero mapping quality?}

\textbf{Q10: Which of the first or second reads in the fragment are higher
base quality on average?}

    \textbf{Congratulations}, you have succesfully aligned some NGS data to a
...
looking at the alignment of reads onto a reference genome.
...
information about its alignment.
    ...
    \begin{figure}
\centering
\includegraphics{images/igv-read-information.png}
...
confidence when it is not really there. This can have an effect on
downstream analyses such as variant calling.
...
In a web browser open the file called \texttt{plots/index.html} to view the QC
information.
...
\end{tcolorbox}
    ...
    We've seen that there are two directories called \texttt{lane1} and
\texttt{lane2} that correspond to the two sequencing lanes produced from this
sequencing library. In order to mark library PCR duplicates, we first need to
merge the two lane BAM files. We will use the picard tool called `MergeSamFiles'
